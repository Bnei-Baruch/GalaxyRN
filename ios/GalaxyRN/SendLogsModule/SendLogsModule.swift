...
@objcMembers public class SendLogsModuleImpl: NSObject {
    private static let TAG = "SendLogsModule"
    private static let MAX_LINES = 1000
    private static let PACKAGE_NAME = "com.galaxy_mobile"

    public func sendLogs(_ email: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        ...
        if #available(iOS 15.0, *) {
            do {
                ...
                for entry in entries {
                    if lineCount >= SendLogsModuleImpl.MAX_LINES {
                        break
                    }
                    ...
                    let entryString = "\(logEntry.date) [\(subsystem)] [\(category)] \(message)"

                    if subsystem.contains(SendLogsModuleImpl.PACKAGE_NAME) ||
                ...
                if lineCount == 0 {
                    ...
                    for entry in entries {
                        ...
                        allLogs += "\(logEntry.date) [\(logEntry.subsystem)] [\(logEntry.category)] \(logEntry.composedMessage)\n"
                        count += 1
                    }
                    logs = allLogs
                }
    ...
    private func buildLogHeader() -> String {
        ...
        header += "Package: \(SendLogsModuleImpl.PACKAGE_NAME)\n"
