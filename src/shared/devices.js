...
import {mediaDevices, getUserMedia} from "react-native-webrtc";
...
class LocalDevices {
  ...
  init = async () => {
    ...
    // }
    const media = await mediaDevices.getUserMedia({audio: true, video: true})

  }

  initt = async (onChange) => {
    let devices = [], ts = 0;
    ...
    let storage_video = null
    let storage_audio = null
    let storage_setting = null
    ...
    if (this.video.error) {
      ...
      devices = await mediaDevices.enumerateDevices();
      this.audio.devices = devices.filter((a) => !!a.deviceId && a.kind === "audioinput");
      console.log(this.audio.devices)
      this.video.devices = devices.filter((v) => !!v.deviceId && v.kind === "videoinput");
    ...
    mediaDevices.ondevicechange = async(e) => {
