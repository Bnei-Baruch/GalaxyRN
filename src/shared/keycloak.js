...
class Keycloak {
  ...
  setSessopn = (data) => {
    ...
    RNSecureStorage.setItem('user_session', JSON.stringify(session), {accessible: ACCESSIBLE.WHEN_UNLOCKED});
  }

  Logout = (callback) => {
    if(this.ival) clearInterval(this.ival);
    logout(authConfig, {idToken: this.session.idToken, postLogoutRedirectUrl: 'com.galaxy://callback'})
      .then(res => {
  ...
  getSession = () => {
    RNSecureStorage.getItem('user_session')
      .then(s => {
  ...
  tokenMonitor = () => {
    ...
    this.ival = setInterval(() => {
      ...
      if(payload) {
        log.debug("_tokenMonitor: time: ", new Date(payload.exp * 1000) - new Date);
    ...
    RNSecureStorage.getItem('user_session')
      .then(data => {
        if(data) {
          ...
          const token_expired = new Date(session.payload.exp * 1000) - new Date < 0;
          if(token_expired) {
