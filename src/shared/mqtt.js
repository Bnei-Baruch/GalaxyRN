...
class MqttMsg {
  ...
  init = (user, callback) => {
    ...
    const transformUrl = (url, options, client) => {
      client.options.clientId = service ? user.id : user.id + '-' +
        randomString(3)
  ...
  send = (message, retain, topic, rxTopic, user) => {
    ...
    log.debug(
      '%c[mqtt] --> send message' + cd + ' | topic: ' + topic + ' | data: ' +
      message, 'color: darkgrey')
  ...
  watch = (callback) => {
    this.mq.on('message', (topic, data, packet) => {
      ...
      log.debug('%c[mqtt] <-- receive message' + cd + ' | topic : ' + topic,
        'color: darkgrey')
      const t = topic.split('/')
      if (t[0] === 'msg') t.shift()
      ...
      switch (root) {
        ...
        case 'janus':
          ...
          const mit = json?.session_id ||
            packet?.properties?.userProperties?.mit || service
