...
class JanusStream {
  constructor() {
    ...
    this.videos    = Number(RNSecureStorage.getItem('vrt_video')) || 1;
    this.audios    = Number(RNSecureStorage.getItem('vrt_lang')) || 2;
    this.mixvolume = null;
    this.talking   = null;
    this.config    = null;
    /*
  ...
  initStreaming = (srv) => {
    ...
    const promise = new Promise((resolve, reject) => {
      this.initJanus(srv, async () => {
        const promises = [];
        if (!this.videoJanusStream) {
          promises.push(this.initVideoStream());
        }
        if (!this.audioJanusStream) {
          promises.push(this.initAudioStream());
        }
        if (!this.trlAudioJanusStream) {
          //let id = await RNSecureStorage.getItem('vrt_langtext')|| 301;
          const id = 301;
          promises.push(this.initTranslationStream(id));
        }
  ...
  initVideoStream = () => {
    ...
    return this.janus.attach(this.videoJanusStream).then((data) => {
      log.debug('[shidur] attach video', data);
      return this.videoJanusStream.watch(this.videos).then((stream) => {
  ...
  initAudioStream = () => {
    ...
    return this.janus.attach(this.audioJanusStream).then((data) => {
      log.debug('[shidur] attach audio', data);
      return this.audioJanusStream.watch(this.audios).then((stream) => {
  ...
  initTranslationStream = (streamId) => {
    ...
    return this.janus.attach(this.trlAudioJanusStream).then((data) => {
      log.debug('[shidur] attach translation', data);
      return this.trlAudioJanusStream.watch(streamId).then((stream) => {
